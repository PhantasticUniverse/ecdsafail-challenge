...
fn active_width(step: usize) -> usize {
    let ideal = 256.0 - (step as f64) * 0.711 + 25.0;
...
fn gcd_predict(mut u: U256, mut v: U256) -> Option<GcdPrediction> {
    let mut log = Vec::with_capacity(393);
    for step in 0..393 {
...
fn add_cleanup_ok(acc: U256, a: U256) -> Option<U256> {
    ...
    if !overflow && final_acc >= SECP256K1_P {
        return None;
    }
...
fn predict_apply_product(log: &[(bool, bool)], mut x: U256, mut y: U256) -> bool {
    for &(b0, b0_and_b1) in log.iter().rev() {
        y = dblm(y);
        if b0 {
...
fn predict_apply_quotient(log: &[(bool, bool)], mut x: U256, mut y: U256) -> bool {
    for &(b0, b0_and_b1) in log {
        if b0_and_b1 {
            std::mem::swap(&mut x, &mut y);
        }
        if b0 {
